...
from .uad import SM
class ISM(SM):
    def __init__(self, C=0.01, eta = 0.99, r = None):
        super().__init__()
        self.C = C
        self.eta = eta
        self.r = r
    def fit(self, train_nX, train_aX):
        #共分散行列の作成
        A = train_nX.T@train_nX/train_nX.shape[0]-self.C*train_aX.T@train_aX/train_aX.shape[0]
        #固有値問題
        ...
        if self.r>=train_nX.shape[1]:
            self.r = train_nX.shape[1]-1
        #基底ベクトルの選択
...
class ConvexSSAD:
    """ URL : https://github.com/lukasruff/Deep-SAD-PyTorch/blob/master/src/baselines/shallow_ssad/ssad_convex.py
        Paper : https://arxiv.org/pdf/1401.6424.pdf
...
class SSAD(object):
    def __init__(self, kernel='linear', d = 2, gamma = 3.0, kappa = 1.0, Cp = 1.0, Cu = 1.0, Cn =1.0):
        """Init SSAD instance"""
    ...
    def fit(self, train_nX, train_aX):
        X = np.r_[train_nX, train_aX]
        Y = np.r_[np.ones(train_nX.shape[0]), (-1)*np.ones(train_aX.shape[0])]

        self.train_X = X
        K = self._kernel_matrix(X)
        self.model = ConvexSSAD(kernel=K, y=Y, kappa=self.kappa, Cp=self.Cp, Cu=self.Cu, Cn=self.Cn)
        self.model.fit()

    def decision_function(self, test_X):
        test_K = self._kernel_matrix(test_X, self.train_X)
        return (-1)*self.model.apply(test_K).flatten() #OCSVMと同じ評価値±0で区切り。0以上が正常
